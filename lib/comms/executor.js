...
async function actionCreateClaim({ message, extractedData }) {
  ...
  const newClaim = {
    ...
    // status remains 'Open' (the schema default) for compatibility
    // with the legacy status filter on the claims list. Lifecycle
    // phase is the new field that distinguishes a freshly intimated
    // claim from a formally registered one — it always starts at
    // 'intimation' for claims auto-created by the comms pipeline.
    status: 'Open',
    phase: 'intimation',
  };
