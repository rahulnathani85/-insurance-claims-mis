...
import { dualWriteClaimFields } from '@/lib/provenance/dualWrite';
import { captureError } from '@/lib/observability';
import { generateReplyDraft } from './replyGenerator';
import { assertCommsEnabled } from './killSwitch';
...
async function actionCreateClaim({ message, extractedData, overrides, attachments = [] }) {
     const { paused } = await assertCommsEnabled('auto_create_claim_paused');
     if (paused) {
       return { status: 'skipped', error: 'soft_kill_auto_create_claim' };
     }
   
     // Approve-time overrides from the review queue (modifications 30-04-2026 §2):
     //  - overrides.lob lets the clerk pick the IRDAI LOB at tag time
     const lob = normaliseLob(overrides?.lob || extractedData.lob);
  ...
  // Provenance dual-write — every extracted field that's provenance-
  // managed gets a claim_field_values row tagged source_type='email',
  // documentType='incoming_email', extractedBy='ai:extract+ocr@v1' so
  // the FieldWithProvenance dot in the UI can show the surveyor where
  // each value came from. Failures here are NON-FATAL — the legacy
  // claims columns are already written so the claim is usable; the
  // provenance ledger is observability for now.
  //
  // Maps the AI-pack-shape extractedData keys to the portal column
  // names that PROVENANCE_MANAGED_FIELDS lists (lib/provenance/dualWrite.js).
  // Fields outside that list (e.g. claim_number, intake_email_*) are
  // left as legacy column-only values for now.
  try {
    const provenanceUpdates = {};
    if (extractedData.insured_name)                      provenanceUpdates.insured_name = extractedData.insured_name;
    if (extractedData.policy_no || extractedData.policy_number) {
      provenanceUpdates.policy_number = extractedData.policy_no || extractedData.policy_number;
    }
    if (extractedData.date_of_loss || extractedData.date_loss) {
      provenanceUpdates.date_loss = extractedData.date_of_loss || extractedData.date_loss;
    }
    if (extractedData.location || extractedData.loss_location) {
      provenanceUpdates.loss_location = extractedData.location || extractedData.loss_location;
    }
    if (lob) provenanceUpdates.lob = lob;
    if (extractedData.insurer_name || extractedData.insurer) {
      provenanceUpdates.insurer_name = extractedData.insurer_name || extractedData.insurer;
    }
    if (extractedData.estimated_loss_amount || extractedData.estimated_loss) {
      provenanceUpdates.estimated_loss_amount = extractedData.estimated_loss_amount || extractedData.estimated_loss;
    }

    if (Object.keys(provenanceUpdates).length > 0) {
      await dualWriteClaimFields(supabaseAdmin, created.id, provenanceUpdates, {
        type: 'email',
        documentType: 'incoming_email',
        label: `Auto-extracted from email "${message.subject || '(no subject)'}"`.slice(0, 200),
        extractedBy: 'ai:extract+ocr@v1',
        // Per-field confidence isn't surfaced by the extractor today
        // (it returns parsed values + a single is_valid flag). Leaving
        // null is honest — surveyors see "(blank)" confidence in the
        // popover and know the value should be reviewed.
        confidence: null,
        sourceId: String(message.id),
        capturedBy: null,  // auto-routed; no human in the loop here
      });
    }
  } catch (e) {
    captureError(e, {
      area: 'comms-executor-create-claim-provenance',
      claim_id: created.id,
      message_id: message.id,
    });
    // Swallow — claim is already created, provenance is observability.
  }

  // Materialise email attachments as claim_documents rows so the unified
  // Documents tab on /claim-detail/<id> shows what arrived with the
  // intimation. Without this, attachments only live in message_attachments
  // (the immutable comms-audit record) and never surface on the claim's
  // Documents view. attachment_id is the back-link + dedup key.
  if (Array.isArray(attachments) && attachments.length > 0) {
    try {
      const docRows = attachments.map((a) => ({
        claim_id: created.id,
        ref_number: created.ref_number || null,
        attachment_id: a.id,
        ...
        gmail_date: message.received_at || null,
        company,
        status: 'Uploaded',
      }));
      ...
      captureError(e, {
        area: 'comms-executor-create-claim-attachments',
        claim_id: created.id,
        message_id: message.id,
      });
      // Swallow — claim is already created.
    }
  }
...
async function actionFileToClaimFolder({ message, claim, attachments }) {
  ...
  // Skip attachments already filed for this claim — the partial-unique
  // index on (claim_id, attachment_id) would reject duplicates anyway,
  // but this avoids pulling the whole insert down on a single retry.
  ...
  if (attachmentIds.length > 0) {
    ...
    alreadyFiled = new Set((existing || []).map((r) => r.attachment_id));
  }

  const rows = attachments
    .filter((a) => !alreadyFiled.has(a.id))
    .map((a) => ({
      claim_id: claim.id,
      ref_number: claim.ref_number || null,
  ...
  if (rows.length === 0) {
    return { status: 'skipped', error: 'all attachments already filed', payload: { claim_id: claim.id, filed_count: 0 } };
  }
...
// ------------------------------------------------------------------
export async function executeRouting(messageId, { triggeredBy = 'auto', overrides = null } = {}) {
  ...
  // tag_definitions.routing_actions can hold entries in two shapes:
  //   - legacy:  "draft_reply"
  //   - current: { action_type: "create_claim", params: { ... } }
  // We support both. The action-type STRING is what HANDLERS is keyed on;
  // the params (if provided) are passed through to the handler as `payload`.
  const routingActions = tagDef?.routing_actions || [];
  const execResults = [];
  let resolvedClaimId = message.claim_id || claim?.id || null;

  for (const actionConfig of routingActions) {
    const actionType =
      typeof actionConfig === 'string' ? actionConfig : actionConfig?.action_type;
    const params =
      typeof actionConfig === 'object' && actionConfig?.params ? actionConfig.params : {};

    const handler = actionType ? HANDLERS[actionType] : null;
    if (!handler) {
      execResults.push({ actionType, status: 'skipped', error: 'unknown action' });
      // Keep routing_executions in sync so "skipped" still shows up in audit.
    ...
    await supabaseAdmin.from('routing_executions').insert([{
      ...
      payload: result.payload || params || null,
