...
//   - Shared mailbox  (tokenRow.is_comms_mailbox  === true)
//       Uses COMMS_GMAIL_CLIENT_ID / _SECRET (separate OAuth app)
//   - Per-user opt-in (tokenRow.is_comms_opted_in === true)
//       Reuses the existing GMAIL_CLIENT_ID / _SECRET OAuth app
//       (the module just has its own redirect URI)
//
...
// ------------------------------------------------------------
// Pick OAuth client credentials based on which flow owns the row.
// Shared mailboxes use the new Comms OAuth app.
// Per-user opted-in rows use the same OAuth app as the existing
// user-facing Gmail connect feature.
// ------------------------------------------------------------
function getClientCredsForRow(tokenRow) {
  if (tokenRow?.is_comms_mailbox) {
    return {
      clientId: process.env.COMMS_GMAIL_CLIENT_ID,
      clientSecret: process.env.COMMS_GMAIL_CLIENT_SECRET,
      mode: 'shared',
    };
  }
  return {
