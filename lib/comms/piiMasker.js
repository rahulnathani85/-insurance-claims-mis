...
// then forwards to lib/aiClient.callAI() (legacy single-provider).
// Returns { text, provider, redactions }.
//
// Used by the legacy classifier (lib/comms/classifier.js). Stage 3c
// retires that classifier; new code should use wrapCallLLM instead.
...
// wrapCallLLM — same masking, but routes to the new lib/aiClients
// callLLM (Claude primary, Gemini fallback, ai_call_log tracking).
// Forwards `messageId`, `attachmentId`, `triggeredBy` so the call
// gets attributed in ai_call_log.
//
// Used by lib/comms/triageExtractor (Stage 3c+).
export async function wrapCallLLM({
  systemPrompt,
  messages,
  maxTokens,
  ...llmOpts // provider, model, fallbackProviders, messageId, attachmentId, triggeredBy
}) {
  let totalRedactions = 0;
