...
import { isPlaceholderRef } from './refNumber';
...
export function validateRegistration(claim, options = {}) {
  ...
  // 4. ref_number must not be the INTAKE/<co>/<msgid> placeholder at registration
  // time — that's a stop-gap used only while phase='intimation'. The clerk
  // assigns a real surveyor reference (manual or via the auto-generator) on
  // the Pending Registration page before clicking Register.
  if (isPlaceholderRef(claim?.ref_number)) {
    errors.push(
      'ref_number is still the INTAKE/ placeholder — assign a real surveyor reference before registering'
    );
  }
